...
//Code Here
let myStrongest = monstersInYourPocket.filter( (currentValue) => currentValue.CP > 200)
...
let orderTotals = orders.map((element) => element.price * (element.tax + 1))
...
let bobsTotal = purchases.filter( (element) => element.owner === 'Bob').map((element) => element.price).reduce( (total, currentValue) => total + currentValue )
